...
# interpolate xmus
xmus_calculated = dict()
xmus_calculated["E"]=e_int
for i in xmus_calculated:
    f = InterpolatedUnivariateSpline(i[0],i[1])
    i_int = f(e_int)
    xmus_calculated[f"site_{atoms_calculated[i][2]}_mu"]=i_int
    
   
# weight-averaged total XAS
xmu_total = 0
for i in range(len(occupancy)):
    xmu_total = xmu_total + xmus_calculated[f"site_{atoms_calculated[i][2]}_mu"]*occupancy[i] 
xmu_total = xmu_total/sum(occupancy)

xmus_calculated["total_mu"]=xmu_total    
spectra=pd.DataFrame(xmus_calculated)
...
#ind is index of files in the folder input
#np.save(f'./feff_{ind}/_atoms_calculated', atoms_calculated) 
atoms_calculated.to_parquet(f'{filename[ind][6:-4]}_out.gzip',
              compression='gzip')
