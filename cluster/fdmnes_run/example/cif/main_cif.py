...
def fdmnes_calculator_mpi(js,ncores,host_ind):
    ...
    try:
        ...
        print(host)
        for e in exe_list:
            _ = subprocess.run(['HOST_NUM_FOR_MUMPS=12  bash %s/%s -np %d --hostfile %s --host %s >> fdmnes.out ' %(exe_path,e,ncores,'hostfile',host)],shell=True)
...
with open(path+'js/js_'+cif_files[ind][-20:-4]+'.json', 'w') as f:
    json.dump(js_out, f)
